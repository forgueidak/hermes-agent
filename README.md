...
> **Note:** I set `REQUEST_TIMEOUT` to `120` seconds — local Ollama inference can be slow on CPU, so the default `30` was timing out regularly for me.

> **Note:** `CONTEXT_WINDOW` is set to `8192` to match the default Ollama context size. If you've configured Ollama with a larger context (e.g. `num_ctx=16384`), bump this to match or the model won't use the full window.
